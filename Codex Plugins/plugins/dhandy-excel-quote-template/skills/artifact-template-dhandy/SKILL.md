...
- Cover summary: use `B:L` with `Code B:C`, `Item D:F`, `Basis G:I`, and `Amount J:L`. Apply one indent level to left-aligned item/basis cells and right-aligned amount cells in body rows.
  - Keep all four summary-header labels centered. Use the English label `CODE` in the first header cell; the Korean `코드` glyph creates a false underline/stair-step artifact in Excel PDF rendering at this size. In body rows, keep code centered, item/basis left aligned, and amount right aligned.
